				...
				if(file_exists("../../modules/" . $mod["mod_path"] . "/inc_security.php")){
					...
					<tbody>
						<tr <? if(intval($mod["mod_id"]) != intval($module_id)){?> bgcolor="#FFCC00"<? }?>>
							<td width="10"><?=$i?></td>
							<td>
								<font size="+1" color="#FF0000"><?=$mod["mod_name"]?></font> <input type="submit" value="Save" class="form">
								<img src="<?=$fs_imagepath?>delete.png" alt="DELETE" border="0" onClick="if (confirm('Bạn muốn xóa?')) { window.location.href='deletemodule.php?record_id=<?=$mod["mod_id"];?>&returnurl=<?=base64_encode(getURL())?>'}" style="cursor:pointer">
							</td>
						</tr>
					</tbody>
