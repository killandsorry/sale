...
function checkAccessCategory(){
	...
	if($row = mysqli_fetch_assoc($db_category->result)){
		...
		if($row["adm_isadmin"] == 1) {
			$db_getall = new db_query("SELECT cat_id FROM category");
...
function checkDeleteCategory($record_id){
	...
	$db_module = new db_query("SELECT cat_type FROM categories_multi WHERE cat_id = " . intval($record_id));
...
function save_log($filename, $log_content){
	$path = $_SERVER['DOCUMENT_ROOT'] . "/logss/";
