...
<?=template_top(translate_text("Member management listing"))?>
		...
		<table width="100%" border="1" cellpadding="3" cellspacing="0" class="table" bordercolor="<?=$fs_border?>">
			...
			<td align="center" class="bold bg" nowrap="nowrap"><?=translate_text("Full name")?></td>
			...
			<td width="10" align="center" class="bold bg"><img src="../images/delete.gif" border="0"></td>
			</tr>
			<?
			$countno = 0;
			...
			?>
			  <tr <? if($countno%2==0) echo ' bgcolor="#FAFAFA"';?>> 
				...
				<td align="center">
					...
					while ($row_channel = mysqli_fetch_assoc($db_access->result)){
				...
				<td align="center"><img src="<?=$fs_imagepath?>delete.png" border="0" onClick="if (confirm('<?=translate_text("Are you sure to delete")?>?')){ window.location.href='active.php?record_id=<?=$row["adm_id"]?>&type=adm_delete&value=<?=abs($row["adm_delete"]-1)?>&url=<?=base64_encode(getURL())?>'}" style="cursor:pointer"></td>
			  </tr>
			<? } ?>
		</table>
		<? /*---------Body------------*/ ?>
