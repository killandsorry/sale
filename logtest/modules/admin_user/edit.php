...
<?=template_top(translate_text("Edit member"))?>
		<? /*---------Body------------*/ ?>
		<table cellpadding="5" cellspacing="0" width="100%" class="table" border="1" bordercolor="<?=$fs_border?>">
			<tr>
				<td valign="top" class="bold bg">
					<?=translate_text("Edit member profile")?>
				</td>
				<td valign="top" class="bold bg">
					<?=translate_text("Change password member")?>
				</td>
			</tr>
			<tr>
				<td>
						<? $row = mysqli_fetch_assoc($db_admin_sel->result); ?>
						<form ACTION="<?=$ff_action;?>" METHOD="POST" name="edit_user">
								<table align="center" cellpadding="4" cellspacing="0" border="0">
									 ...
									 <tr <?=$fs_change_bg?>> 
										...
										<td> <input type="hidden" name="adm_edit_all" <? if($row["adm_edit_all"]==1) echo 'checked="checked"';?> value="1" >
											...
											<table cellpadding="2" cellspacing="0">
												<tr>
												<?
												while ($lan=mysqli_fetch_assoc($db_getall_languages->result)){
												?>
									...
									<tr> 
										...
										<td>
											<input type="button" class="form" onClick="document.edit_user.submit();" value="<?=translate_text("save_change")?>">
										</td>
									</tr>
								</table>
						...
						<form ACTION="<?=$ff_action;?>?iAdm=<?=$iAdm?>" METHOD="POST" name="edit_password" onSubmit="formchangepass(); return false;">
								<table align="center" cellpadding="4" cellspacing="1" bordercolor="#CCCCCC" border="1" style="border-collapse:collapse">
									...
									<tr> 
										<td nowrap align="right"></td>
										<td> 
											<input type="submit" class="button" value="<?=translate_text("Change password")?>" > 
										</td>
									</tr>
								</table>
