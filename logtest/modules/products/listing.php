...
$list->add("pro_name", "Tiêu đề", "str", 0, 0);
...
if($keyword != "" && $keyword != translate_text("Enter keyword")){
	$sql .= " AND (
						...
						OR pro_formula LIKE '%" . $keyword . "%'
						OR pro_deportment LIKE '%" . $keyword . "%'
						OR pro_assign LIKE '%" . $keyword . "%'
						OR pro_using LIKE '%" . $keyword . "%'
						OR pro_dosage LIKE '%" . $keyword . "%'
						OR pro_contraindications LIKE '%" . $keyword . "%'
						OR pro_note LIKE '%" . $keyword . "%'
						OR pro_preservation LIKE '%" . $keyword . "%'
						OR pro_production LIKE '%" . $keyword . "%'
						)";
}
if($pro_cat_id > 0){
  ...
  <?
	...
	while($row	=	mysql_fetch_assoc($db_listing->result)){
		...
		?>
			...
			<td width="100">
				<img src="<?=$img_path . "small_" . $row["pro_picture"]?>" onerror="this.src='<?=$fs_nophoto?>'" width="90" height="90" />
			</td>
			<?
			$link		=	$fs_domain . $row["pro_rewrite"] . '.html';
			?>
			<td valign="top">
				...
				<div style="padding-top: 5px;">Giá thị trường: <b style="color: red;"><?= format_number($row["pro_price_market"])?></b>đ</div>
			</td>
			<td width="100" align="left"> <?=isset($arr_cat[$row['pro_cat_id']]) ? $arr_cat[$row['pro_cat_id']] : ''?> </td>
			...
			<td width="10" align="center"><a onclick="if (confirm('Bạn muốn xóa bản ghi?')){ deletefile(<?=$row['pro_id']?>,'<?=$row['pro_picture']?>'); }" href="#" class="delete"><img border="0" src="../../resource/images/grid/delete.gif"></a></td>
			<?=$list->end_tr()?>
		<?
  	}
	  unset($db_listing);
