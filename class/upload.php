...
class upload{
	...
	*/
	...
	function upload($upload_name, $upload_path, $extension_list, $limit_size, $check_image=0, $prefix="", $zip=0){
		...
		$new_filename					= $this->generate_name($_FILES[$upload_name]['name'], $prefix);
		
		$this->original_name			= $_FILES[$upload_name]['name'];
		...
		//Check image type
		if(($check_image != 0) && ($this->getExtension($this->file_name) != "swf")){
